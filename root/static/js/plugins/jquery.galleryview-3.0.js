...
*/
	...
	*/
		function showItem(i,speed,callback) {
			...
			var currentId = $(".gv-frame.current").attr('id');
			console.log("the current offer id is :", currentId);
			presentFragment = Backbone.history.fragment;
			console.log("the present fragment is  :", presentFragment);
			var substr = presentFragment.split('/');
			page = substr[1];
			placeid = substr[2];
			tab = substr[3];
			previousId = substr[4];
			console.log("the previous offerid is ",previousId);
			
			if(currentId === undefined){ // when there are no offers then the url should be set so that back button works . 
				newFragment = "!/"+page+"/"+placeid+"/"+tab;
				
			}
			else{
				newFragment = "!/"+page+"/"+placeid+"/"+tab+"/"+currentId;
			...
			if(previousId == ''|| previousId === undefined){
				...
				console.log("replacing the fragment");
			}
			else{
			...
			if(tab == "offers"){
				interaction[DEAL_ID] =currentId;
			}
			else if(tab == "featured"){
				interaction[FEATURED_ID ] =currentId;
			}
			bodyContent = {};
	...
	*/
		function buildFilmstrip() {
			...
			if(opts.show_filmstrip_nav) {
				...
				if(filmstrip_orientation=='horizontal') {
					console.log("setting the position for the arrows when horizontal",navPrev,navNext);
					navNext.css({					 
